...
export default async function tournament(app: FastifyInstance) {
    ...
    app.post("/submit-score", {
      ...
      schema: {
        body: {
          type: 'object', required: ['matchId', 'score1', 'score2'],
          properties: { matchId: { type: 'integer' }, score1: { type: 'integer' }, score2: { type: 'integer' } },
        }
      }
     }, async (req, reply) => {
      ...
      try {
        const match = await prisma.game.update({ where: { id: matchId }, data: { score1, score2 } });
        
        const tournamentId = match.tournamentId;
        const round = match.round;
        
        const roundMatches: any[] = await prisma.game.findMany({ where: { tournamentId, round } });
        
        const allFinished = roundMatches.every(m => m.score1 !== 0 || m.score2 !== 0);
        if (!allFinished) return reply.send({ messge: "Score submitted! Waiting for other matches." });
        ...
        if (winners.length === 1) {
          ...
          return reply.send({ message: "Tournament complete!", champion: winners[0] });
        }
